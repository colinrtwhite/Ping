...
public class PingSyncAdapter extends AbstractThreadedSyncAdapter {
	...
	private static final String[] projection = {MonitorEntry.TITLE, MonitorEntry.STATUS, MonitorEntry.LAST_NON_ERROR_STATUS};
	...
	public void onPerformSync(Account account,
		...
		if (!(extras.getBoolean(ContentResolver.SYNC_EXTRAS_EXPEDITED, false) &&
				...
				sharedPref.getBoolean(getContext().getString(R.string.pref_key_avoid_metered_connection), false) &&
				((ConnectivityManager) getContext().getSystemService(Context.CONNECTIVITY_SERVICE)).isActiveNetworkMetered()) {
	...
	private void triggerNotification(int monitorId, String title, String url, int status, long timeLastChecked) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			notificationBuilder.setColor(ContextCompat.getColor(getContext(), R.color.primary));
		}

		// Construct artificial back stack.
		TaskStackBuilder stackBuilder = TaskStackBuilder.create(getContext());
		Intent monitorDetailActivityIntent = new Intent(getContext(), MonitorDetailActivity.class);
